...
internal class CodeFlowPullRequestUpdater : PullRequestUpdater
{
        ...
        string prUrl)
    {
        ...
        try
        {
            ...
            SourceDiffVerificationResult result = await _codeflowChangeAnalyzer.VerifyForwardFlowAsync(
            ...
            var comment = BuildSourceDiffVerificationComment(result, subscription.SourceRepository, previousSourceSha, build.Commit);
    ...
    private static string BuildSourceDiffVerificationComment(
        SourceDiffVerificationResult result,
        string sourceRepo,
        string oldSha,
        string newSha)
    {
        ...
        builder.AppendLine();
        builder.AppendLine($"Compared this PR against the source diff `{shortOld}...{shortNew}` of `{sourceRepo}`:");
        builder.AppendLine();

        if (result.Matches)
        {
            builder.AppendLine("✅ This PR matches the source diff.");
            builder.AppendLine();
        }

        builder.AppendLine($"- ✅ Applied: {result.AppliedFiles.Count} files");
        builder.AppendLine($"- ➖ No-op (VMR already at target): {result.NoOpFiles.Count} files");
        builder.AppendLine($"- ⚠️ Changes don't match: {result.MismatchedFiles.Count} files");

        if (result.MismatchedFiles.Count > 0)
        {
            foreach (var file in result.MismatchedFiles)
            {
                builder.AppendLine($"  - `{file}`");
            }
        }

        builder.AppendLine($"- ⚠️ Unexpected (changed in PR, not in source diff): {result.UnexpectedFiles.Count} files");

        if (result.UnexpectedFiles.Count > 0)
        {
            foreach (var file in result.UnexpectedFiles)
            {
                builder.AppendLine($"  - `{file}`");
            }
        }
