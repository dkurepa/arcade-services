...
Empty flag set => "PR matches the source diff." Otherwise list the mismatching
files. The comment renders a per-file status, e.g.:

```
Verified this PR against source diff 60e46e1...5932bea (dotnet/wpf):
  ✅  applied        57 files
  ➖  no-op           3 files (VMR already at target)
  ⚠️  changes differ  0 files
  ⚠️  unexpected      0 files (in PR, not in source diff)
```
